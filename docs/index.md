# Alpha Auto Research：起飞，全自动长周期强化学习科研实验

> 我们构建了一个 Alpha Auto Research 系统，让 AI Agent 自主完成「提出假设 - 设计实验 - 调度训练 - 分析结果 - 撰写报告」的完整研究闭环。本文记录了六个由 AI Agent 自主完成的研究课题——从 AppWorld 超参搜索，到 LoRA 配置优化、多模型规模对比、医疗对话训练，以及训练异常检测机制研究——展示这套系统如何在无人值守的情况下，一夜之间跑完多组对比实验，并给出令人信服的结论。
...
但这些系统大多停留在「写论文」的层面。我们想做的不太一样——**让 AI 替你跑实验**。具体来说，Alpha Auto Research 瞄准的是以下场景：

1. **长周期实验闭环**。单次 RL 训练动辄数小时甚至数天，不是跑个 MATH-500 benchmark 就能出结论的。系统需要自主完成「提出假设 → 设计实验 → 调度训练 → 等待数小时 → 分析结果 → 决定下一步」的完整闭环，而且全程无人值守。
2. **榨干集群带宽**。训练实验批量运行：当你有好几个显卡时（或者GPU集群时），Auto Research 应该能最大效率地并行调度多组实验，而不是一个一个排队跑。
...
4. **成本低到可以「随便跑」**。驱动 Agent 的大模型我们用 MiniMax M2.7 或 GLM 的 coding plan——你是不是也觉得晚上没榨干 coding plan 血亏一个亿？一个 Auto Research 轻松帮你把 coding plan 的剩余额度用得干干净净。当 API 费用几乎为零，「要不要再多跑一组实验」就不再是一个需要纠结的问题。
5. **全自动与半人工介入，随时可切**。我们提出了「实验蓝图」机制：由科研主管 Agent 编写结构化的实验蓝图，确定实验的大体方向和关键参数，然后交给集群上的子智能体去具体执行——改 bug、调配置、启动训练。关键在于，每个环节都可以停下来人工介入：审核蓝图、修改参数、甚至直接接管某个子任务。整个过程完全透明，不会因为实验周期长达数天而在错误方向上白白浪费算力。


## 二、Auto Research：让 Agent 成为研究者
...
在介绍架构之前，先说一下我们的技术选型——这直接决定了这套系统能否低成本地跑起来。
...
- **极其便宜**：M2.7 的 API 调用价格只有顶级闭源模型的几十分之一。让 AI 跑一整夜帮你做研究，API 费用可能还不到一杯咖啡钱
...
这个选择背后的逻辑很简单：Auto Research 系统中，Agent 需要持续运行数小时甚至数天，期间会产生大量的 API 调用。如果用顶级模型，光 token 消耗就是一笔不小的开支，会让人犹豫「要不要再多跑一组实验」。而用 M2.7，这个顾虑完全不存在——**便宜到可以放心让 AI 反复试错**。

**强化学习训练框架：[AgentJet](https://github.com/modelscope/AgentJet)**。Auto Research 负责「指挥研究」，而实际的 GPU 训练则由 AgentJet 完成。AgentJet 是由通义团队开源（Apache 2.0）的强化学习训练框架，专门用于优化大模型驱动的 Agent。它支持多种任务场景——数学推理、AppWorld 环境交互、医疗对话、甚至狼人杀博弈——并且原生支持多 GPU 分布式训练和 LoRA 微调。对我们来说，AgentJet 最重要的特性是**稳定可靠**：它能在 8 卡 GPU 集群上连续训练十几个小时而不崩溃，配合 Auto Research 的自动恢复机制，构成了整个系统「跑得动、跑得稳」的基础。

### 2.2 Leader Agent：研究的大脑

Leader Agent 是整个系统的核心。它接收一个用自然语言描述的研究课题，然后自主完成以下工作：

1. **拆解课题**：理解需要研究什么，确定哪些变量需要比较、哪些需要控制
2. **设计分阶段实验**：粗筛 → 细筛的渐进式搜索策略，提前规划好「如果结果是 A 就怎么做，如果结果是 B 就怎么做」的决策分支
3. **生成实验蓝图（Blueprint）**：为每个实验写一份详细的「施工图」（详见 2.4 节），包含运行所需的全部信息
4. **调度执行**：把蓝图发送到 GPU 集群，让多个实验同时开跑
5. **轮询监控**：每 10 分钟检查一次各实验是否完成
6. **收集分析**：读取实验结果，画对比图表，撰写结论
7. **迭代或终止**：对照事先设定的决策树，判断是需要再做一轮更精细的实验，还是结论已经足够清晰可以直接写报告

整个过程中，Leader 会持续记录决策日志，确保任何时候都能回溯「它为什么做了这个决定」。

### 2.3 Worker Agent：实验的双手

每个 Worker Agent 运行在独立的 GPU 服务器上，就像一个尽职的实验室助手：

- **照着蓝图准备环境**：安装所需的软件、配置运行参数
- **启动训练并持续盯着**：以越来越长的间隔检查训练进度（30 秒、1 分钟、2 分钟……逐渐放宽），但如果训练突然崩溃，能立即发现
- **遇到问题尝试自己修**：GPU 资源冲突？重新分配。训练进程挂了？尝试重启。修不了的问题也会如实记录下来
- **无论结果如何都汇报**：训练结束后标记「完成」，确保 Leader 不会无限等待一个已经失败的实验

### 2.4 关键设计：Blueprint（实验蓝图）作为「合约」
...
Blueprint 是 Leader 和 Worker 之间的通信协议，也是整个系统最核心的设计。你可以把它理解为一份**「施工图纸」**——Leader 负责画图纸，Worker 照着图纸干活。
...
| **配置文件** | 训练的详细参数（学习率、GPU 数量等）写在哪个文件里？ |
| **启动命令** | 用什么命令启动训练？ |
...
- **Leader 只需要「想」**，不需要关心执行细节——它生成的蓝图已经包含了 Worker 所需的一切
- **Worker 只需要「做」**，不需要理解研究全局——照着蓝图执行就行
- **实验天然可复现**——蓝图本身就是完整的文档，任何人拿到蓝图都能重新跑出同样的实验

### 2.5 鲁棒性：连续运行数天而不需要人工干预
...
Worker Agent 管理的 GPU 训练进程也有完善的保护机制：防止重复启动同一个实验、能干净地终止卡死的进程、自动检测和清理僵尸进程。就像一个靠谱的运维工程师，默默处理各种意外情况。

**结果：** 在实际运行中，我们的单个实验可以稳健运行数天。中间即使遇到 GPU 争抢、大模型 API 限流、网络波动，系统都能自行恢复并继续工作，真正做到了**「睡前启动，醒来看报告」**。
...
Leader 生成了三份 Blueprint，分别对应 `max_steps = 5, 15, 25`，然后通过 PAI Runner 将它们同时提交到阿里云 GPU 集群。

三个 Worker Agent 在各自的节点上独立工作。期间还遇到了一些状况：

- GPU 资源争抢导致部分实验暂停
- 某个实验在 step 31 处因 GPU 分配冲突需要重启
- `max_steps=5` 的实验一度卡在某个 step

Worker Agent 自动处理了这些异常——重启 Ray 集群、重新分配资源、恢复训练进度。**没有任何人工干预。**

### 3.4 结果：出人意料又合理

训练过夜完成后（每组跑了约 50/80 步，耗时 3.5~8.3 小时不等），Leader Agent 收集了所有验证结果，绘制了对比图表，写出了最终报告。
...
1. **max_steps=5 完全不可用**——Agent 在 5 步内无法完成任何 AppWorld 任务，训练 50 步后通过率仍为 0%。这不是「差一点」，而是根本不够。

2. **max_steps=15 和 25 效果几乎一样**——最终通过率仅差 0.44%，在统计误差范围内。平均奖励也收敛到几乎相同的值（0.667 vs 0.669）。

3. **但 15 比 25 快了近 40%**——每步训练时间从 ~10 分钟降到 ~7 分钟。效率得分（效果提升/训练时间）是 25 的 **1.87 倍**。

Leader Agent 对照预设的决策树，识别出结果符合「**5 << 15 ≈ 25**」的模式（收益递减），直接给出了推荐：

> **最终推荐：max_steps = 15**。在效果与 max_steps=25 持平的情况下，训练速度提升约 30%，效率是后者的 1.87 倍。
...
在实验开始之前就定义好「什么结果意味着什么结论」，这是好的研究者的习惯。它避免了事后看着数据「强行解释」的倾向，也让整个流程可以自动化推进。
...
最终报告中明确列出了实验的局限性：训练未达到计划的 80 步、每种配置只跑了一次缺乏统计显著性验证、训练曲线存在波动……这种自我批评的能力，对于产出可信的研究结论至关重要。
...
- **阿里云灵骏 PAI（`--runner=pai`）**：通过阿里云 PAI DLC 平台提交实验，自动克隆模板任务并分配 GPU 资源。适合需要弹性扩缩容的场景——比如同时跑 6 组实验，不必担心本地机器不够用。本文中 AppWorld 超参搜索的三组并行实验，就是通过 PAI 后端在云端同时调度的。
- **SSH 模式（`--runner=ssh`）**：直接通过 SSH 连接到你自己的 GPU 服务器，在 tmux 会话中运行实验。适合已有固定硬件的团队，零额外成本。系统还会自动配置免密登录，省去手动设置的麻烦。

无论使用哪种后端，Leader Agent 的操作方式完全相同——提交蓝图、等待结果、收集数据。切换后端只需要改一个参数，对整个研究流程零影响。这也意味着你可以在本地 SSH 服务器上调试跑通一个小实验，然后同样的研究课题换成 `--runner=pai` 就能无缝扩展到云端大规模运行。
...
之后就是去睡觉，第二天早上回来看报告。系统的鲁棒性设计（详见 2.5 节）确保了即使中间大模型 API 被限流、GPU 资源被抢占，系统也能自行恢复——这不是理论上的承诺，而是在这次实验中实际发生并被验证过的。


## 六、这意味着什么

Auto Research 不是要取代研究者，而是在处理那些**结构清晰、计算密集、决策规则可形式化**的研究子任务时，将人类从重复劳动中解放出来。

超参搜索就是一个典型场景：问题定义明确，实验流程标准化，结果分析有客观指标。人类研究者最宝贵的时间应该花在提出好的研究问题和解释意外结果上，而不是花在 SSH 进服务器检查训练有没有挂掉。
...
但即便是当前这个版本，它已经展示了一种可能性：**让 AI Agent 不只是写代码的工具，而是能独立开展研究的同事。** 而且，这一切并不依赖昂贵的顶级大模型——驱动整个系统的 MiniMax M2.7 是一个廉价、开放、易获取的国产模型，跑完本文全部六个研究课题的 Agent API 调用费用加起来，可能还不够买一杯咖啡。**高性价比的自动科研，不再是大厂的专利。**
...
> 本文所述实验由 `alpha_auto_research` 系统全自动完成，研究课题于 2026-04-08 下午提交，最终报告于 2026-04-09 凌晨生成。从课题到结论，全程无人工干预。




## 七、更多课题：AI Agent 的五次独立研究

在 AppWorld 超参搜索之后，我们将同样的 Auto Research 系统应用到了更多研究课题上。以下五个课题均由 AI Agent 自主完成实验设计、训练调度、数据收集与结论分析——全部由廉价的 MiniMax M2.7 模型驱动，没有用到任何昂贵的顶级闭源模型。
...
Agent 设计了三组实验，分别测试比值 0.5、1.0 和 2.0：
...
*实验配置：Qwen2.5-7B-Instruct, GSM8K（1,319题），学习率 3e-5，4 GPU*
...
**研究问题：** 更大的模型训练出来一定更好吗？Agent 对 Qwen3 的三个规模进行了系统对比。
...
**任务描述：** 用给定数字通过四则运算得到目标数——一个经典的数学推理任务。Agent 再次对三个模型规模进行了系统对比。
...
**最令人印象深刻的发现：** Qwen3-8B 从 26.78% 一路提升到 83.64%——接近 **3 倍的性能飞跃**。尽管起点差距巨大（8B 的 26.78% vs 14B 的 57.01%），最终差距缩小到仅约 2%。RL 训练展现出了强大的「拉平」效应。
...
*实验配置：1,024 道测试题，每模型 8 GPU，12 小时上限*
...
**这个课题也真实地展示了自动化研究面临的挑战。** 三个模型的训练均因外部 DashScope API 密钥失效而提前终止——这是 Agent 无法自行修复的「系统边界之外」的问题。但 Agent 的表现仍然值得称赞：
...
1. **14B 又赢了。** 在第三个不同的任务上，Qwen3-14B 再次取得最佳成绩，且奖励标准差持续下降（1.364 → 1.235），说明输出越来越稳定。
...
https://github.com/binary-husky/Alpha-RL-Research.git